...
def model4(y, params):
  ...
  # unpacking parameters
  N, kh, a, b1, b2, p1, p2, a_h, a_p, nn, c, p = params.values()
  # N, kh, a, b1, b2, c, p = params.values() 
  ...
  # Nearest neighbours of prey observing all others
  nn_dist_h = np.argsort(np.hstack((norm_prey, norm_pred[:,np.newaxis])), axis = 1)[:,0:nn,:]
  prey_mask = np.hstack((mask_hh, mask_hp[:,np.newaxis]))
  prey_mask[nn_dist_h] = 1 # sets the union
  mask_hh = prey_mask[:,:-1,:]
  mask_hp = prey_mask[:,-1,:]
  ...
  # Nearest neighbours of the predator
  nn_dist_p = np.argsort(norm_pred)[0:nn,:]
  mask_ph[nn_dist_p] = 1
