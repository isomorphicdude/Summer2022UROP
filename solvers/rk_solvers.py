...
def rk2(f, y0, h, dim, times, params):
  ...
  - f: function(prey, pred, params), the RHS of the DE, returning (dim, 2) ndarray of derivatives
  ...
  for i in tqdm(range(t_len-1)):
    k1 = np.array(f(y[i][0:N], y[i][-1:], params))
    y1 = y[i] + (h/2) * k1
    k2 = f(y1[0:N], y1[-1:], params)
    y[i+1] = y[i] + h * k2
...
def rk4(f, y0, h, dim, times, params):
  ...
  for i in tqdm(range(t_len-1)):
    k1 = np.array(f(y[i][0:N], y[i][-1:], params))
    y1 = y[i] + (h/2) * k1
    k2 = np.array(f(y1[0:N], y1[-1:], params))
    y2 = y[i] + (h/2) * k2
    k3 = np.array(f(y2[0:N], y2[-1:], params))
    y[i+1] = y[i] + h * k3
